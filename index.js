...
function drawBackgroundPolygon(
  distance,
  numOfSkills,
  anglePerSkill,
  centerPoint,
...
function renderPokemonPage(pokemon) {
  ...
  const currentGenNumber =
    currentGen.length === 1 ? currentGen.map((gen) => gen.year + 1) : false;
  if (currentGenNumber) {
    ...
    const currentTrainersId = currentGen
      .map((gen) =>
        gen.trainers.find((trainer) => trainer.participantId === pokemon.id),
      )
      .filter((trainer) => trainer != undefined)
      .map((trainer) => trainer.trainerId);
    const trainerNames = currentTrainersId
      .map((trainer) => trainers.find((t) => t.id === trainer))
      .map((trainer) => trainer.name);
    const trainerDisciplineId = currentTrainersId
      .map((trainer) => trainers.find((t) => t.id === trainer))
      .map((trainer) => trainer.disciplineId);
    const disciplineNames = trainerDisciplineId
      .map((trainer) =>
        disciplines.find((discipline) => discipline.id === trainer),
      )
      .map((discipline) => discipline.gymName);
  ...
  const scores = currentGen
    .flatMap((gen) => gen.competitionDays)
    .flatMap((day) => day.events)
    .flatMap((event) => event.scores)
    .filter((score) => score.participantId === pokemon.id)
    .flatMap((id) => id.score);
  ...
  const gameDays = currentGen
    .flatMap((gen) => gen.competitionDays)
    .flatMap((day) => day.events)
    .map((event) => event.scores)
    .filter((score) =>
      score.some((participant) => participant.participantId === pokemon.id),
    );

  let allPlacements = [];
  let gameDayIndex = 0;
  for (let score of scores) {
    const betterParticipants = gameDays[gameDayIndex].filter(
      (game) => game.score > score,
    );
    const myPlacement =
      betterParticipants.length > 0 ? betterParticipants.length + 1 : 1;
  ...
  const yScale = d3
    .scaleLinear()
    .domain([
      d3.min(scoreAndDateData, (data) => data.score) - 50,
      d3.max(scoreAndDateData, (data) => data.score) + 50,
    ])
  ...
  const tooltip = d3
    ...
    .style(
      "box-shadow",
      "0 2px 8px rgba(0,0,0,0.15), 0 4px 20px rgba(0,0,0,0.08)",
    );
  ...
  svg
    ...
    .on("mouseover", function (event, d) {
      tooltip
        .style("display", "block")
        .html(
          `<div id="svg-label-container"><p>${d.date}</p><div id="ball-and-text-svg"><div id="pokemon-ball-svg"></div><p id="score-label-svg">${d.score} points</p></div></div>`,
        );
    })
  ...
  seasons.forEach((gen) => {
    const genOk = gen.competitionDays.some((day) =>
      day.events.some((event) =>
        event.scores.some((score) => score.participantId === pokemon.id),
      ),
    );
    if (genOk) {
